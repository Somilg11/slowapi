...
def use_pipes(*pipes: t.Any) -> t.Callable[[t.Any], t.Any]:
    ...
    def decorator(target: t.Any) -> t.Any:
        existing = tuple(getattr(target, "__slowapi_pipes__", ()))
        target.__slowapi_pipes__ = existing + pipes
        return target
...
def collect_pipes(handler: t.Any, controller: type | None) -> tuple[t.Any, ...]:
    class_level = tuple(getattr(controller, "__slowapi_pipes__", ())) if controller else ()
    return class_level + tuple(getattr(handler, "__slowapi_pipes__", ()))
