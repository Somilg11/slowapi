...
def use_interceptors(*interceptors: t.Any) -> t.Callable[[t.Any], t.Any]:
    ...
    def decorator(target: t.Any) -> t.Any:
        existing = tuple(getattr(target, "__slowfw_interceptors__", ()))
        target.__slowfw_interceptors__ = existing + interceptors
        return target
...
def collect_interceptors(handler: t.Any, controller: type | None) -> tuple[t.Any, ...]:
    class_level = tuple(getattr(controller, "__slowfw_interceptors__", ())) if controller else ()
    return class_level + tuple(getattr(handler, "__slowfw_interceptors__", ()))
...
class EnvelopeInterceptor(Interceptor):
    ...
    Responses that are already :class:`~slowfw.response.Response` objects are
