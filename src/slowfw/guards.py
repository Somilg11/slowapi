...
def use_guards(*guards: t.Any) -> t.Callable[[t.Any], t.Any]:
    ...
    def decorator(target: t.Any) -> t.Any:
        existing = tuple(getattr(target, "__slowfw_guards__", ()))
        target.__slowfw_guards__ = existing + guards
        return target
...
def collect_guards(handler: t.Any, controller: type | None) -> tuple[t.Any, ...]:
    ...
    class_guards = tuple(getattr(controller, "__slowfw_guards__", ())) if controller else ()
    handler_guards = tuple(getattr(handler, "__slowfw_guards__", ()))
