...
class SlowAPI:
    ...
    def __init__(
        ...
        *,
        ...
        self.logger = logger or get_logger("slowfw.app")
        ...
        **meta: t.Any,
        ...
        for method in methods:
            ...
            if controller is not None:
                handler.__slowfw_controller__ = controller  # type: ignore[attr-defined]
    ...
    def include_router(self, router: Router, *, prefix: str = "") -> SlowAPI:
        """Merge a :class:`~slowfw.routing.Router` into the application."""
    ...
    def register_controller(
        ...
        *,
        ...
        version = getattr(controller_class, "__slowfw_version__", None) or (
        ...
        for attribute, route_meta, fn in found:
            ...
            fn.__slowfw_controller__ = controller_class  # type: ignore[attr-defined]
            self.add_route(
                full_path,
                fn,
                [route_meta.method],
                ...
                status_code=route_meta.status_code or getattr(fn, "__slowfw_status_code__", None),
    ...
    def _prepare(self, route: Route) -> _Prepared:
        ...
        controller = getattr(route.handler, "__slowfw_controller__", None)
        signature = analyse(
            route.handler,
...
def _maybe_async(participant: t.Any) -> bool:
    ...
    :func:`slowfw.concurrency.drive`.  The framework's own middleware is
    written that way, and :func:`slowfw.never_suspends` lets user middleware
    make the same promise -- see its docstring for the (real) obligation that
    comes with it.
    """
    if participant is None:
        return False
    if getattr(participant, "__slowfw_never_suspends__", False):
        return False
